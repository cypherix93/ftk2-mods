...
namespace FTK2Mods.Crucible
{
    ...
    internal static class MiniJson
    {
        ...
        /// <summary>
        /// Serializes a tree of <see cref="Dictionary{TKey,TValue}"/> / <see cref="List{T}"/> /
        /// string / bool / numeric / null into JSON.
        ///
        /// Object keys are emitted in **ordinal-sorted** order, not insertion order. That is
        /// load-bearing: <see cref="StateDigest"/> hashes this output, and two peers must produce
        /// byte-identical text for the same logical state or every comparison would report a false
        /// desync. (The reader above is the DevKit original, whose writer preserved insertion order
        /// for the closed parity schema; Crucible's snapshots are reflection-built and have no
        /// guaranteed member order, hence the sort.)
        /// </summary>
        ...
        private static void WriteValue(StringBuilder sb, object value)
        {
            ...
            if (value is float || value is double || value is decimal)
            {
                ...
                // "R" round-trips; invariant culture keeps '.' as the separator on every locale.
                sb.Append(d.ToString("R", CultureInfo.InvariantCulture));
                return;
            }

            JsonNumber number = value as JsonNumber;
            if (number != null) { sb.Append(number.Raw); return; }
            ...
            // Unknown type: stringify rather than throw. A snapshot must never fail to serialize.
